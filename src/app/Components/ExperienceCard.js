...
const ExperienceCard = ({
  ...
  return (
    <div
      className={`flex flex-col items-start ${jobType === 'Graduação' ? 'justify-start' : 'justify-center'} gap-5 md:flex-row`}
      data-aos="fade-right"
    >
      ...
      <div className="text-start max-sm:border-[3px] max-sm:p-5 max-sm:rounded-y-xl max-sm:rounded-r-xl max-sm:border-[rgba(255,255,255,0.1)] max-sm:border-l-[#3DA661]">
        ...
        {isList ? (
          ...
          <p className="text-[rgba(255,255,255,0.5)] max-w-md">{description}</p>
        )}
      </div>
    </div>
  );
